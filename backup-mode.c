...
bool modeIsBackupDeviceAvailable(int backupDevice)
{
	...
	ms = MODE_GetMountStatus();	//GetMountStatus retuns NULL if the MODE response is not right 
	vi = MODE_GetVersionInfo(); //Actually mi and vi will share the same pointer if right, but we are only using data from vi from now one
